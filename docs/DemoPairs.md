...
| `matdcalc` | the `MatD` core on one seeded 4×3 matrix: creation, views and gathers, broadcasting, reductions and axis reductions, the pinned `matmulPure`, masks and `where`, exact `inverse`/`determinant`/`solve`, `svd`, pandas-style `sort`/`argsort`/`percentile`/`rolling`/`diff`/`nlargest`, stacking, CSV text | **any machine** |
