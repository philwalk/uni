...
fn utf16_lines_decode_as_text_not_bytes() {
    ...
    assert_eq!(
        p.try_lines_with(Charset::Utf16Be).unwrap_or_default(),
        ["alpha", "beta"],
        "lines"
    );
    ...
    assert!(
        !p.try_lines_with(Charset::Utf16Be)
            .unwrap_or_default()
            .iter()
            .any(|l| l.contains('\u{0}')),
...
fn utf16_le_and_a_bom() {
    ...
    assert_eq!(
        le.try_lines_with(Charset::Utf16Le).unwrap_or_default(),
        ["alpha", "beta"]
    );
    ...
    assert_eq!(
        p.try_lines_with(Charset::Utf16).unwrap_or_default(),
        ["alpha", "beta"]
    );
    ...
    assert_eq!(
        q.try_lines_with(Charset::Utf16).unwrap_or_default(),
        ["alpha", "beta"],
        "LE BOM"
    );
    ...
    assert_eq!(
        r.try_lines_with(Charset::Utf16).unwrap_or_default(),
        ["alpha", "beta"],
        "defaults to BE"
    );
}
...
fn a_real_but_unsupported_charset_is_refused_loudly() {
    ...
    assert_eq!(
        Charset::by_name("NoSuchCharset").expect("fallback"),
        Charset::Utf8
    );
    // And the supported page keeps working under all its spellings.
    assert_eq!(
        Charset::by_name("windows-1252").expect("1252"),
        Charset::Latin1
    );
