...
fn the_comparison_reads_as_the_name_does() {
    ...
    assert!(
        fresh.newerThan(&old),
        "the newer file IS newerThan the older"
    );
    assert!(
        !old.newerThan(&fresh),
        "the older file is NOT newerThan the newer"
    );
    assert!(
        old.olderThan(&fresh),
        "the older file IS olderThan the newer"
    );
    assert!(
        !fresh.olderThan(&old),
        "the newer file is NOT olderThan the older"
    );
    assert!(
        !old.newerThan(&old),
        "strict: a file is not newer than itself"
    );
}
