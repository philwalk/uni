...
## At parity today (fixture/test-verified, as of 2026-08-18)
...
| `uni.Proc` (`run` ×2, `proc`/`ProcBuilder`, `ProcResult`, `execLines`, `bashExe` `pythonExe` `unameExe` `uname` `isWsl` `osType` `hostname` `where` `whereInPath`, the `!!`/`orElse`/`orFail` extensions, `failFast`) | `uproc` (`run`, `runLines`, `runStream`, `proc`/`ProcBuilder`, `ProcResult`, `execLines`, `StatusExt`, the tool family) | the routing table (`route_cmd(cmd, is_windows)`, both platforms unit-tested), status −1 + message for a program that cannot start, `timeout` → −1, `orLog` for `!!`, `orFail` → `Result<_, i32>` in place of `failFast`; `where` → `whereExe` (keyword) returning `Option`; the streaming callbacks run on the caller's thread via a channel |
...
**Mat[Float]: single precision, bit-for-bit.** `MatF = Mat<f32>` (`udata::matf`) mirrors
the Scala `Mat[Float]` paths: the generic sequential folds in `f32`, `Float.compare` for
the order statistics (`java_float_compare`), IEEE masks widened to `f64` as `cmpIeee`
does, `sqrt`/`std` through `f64` (`MatElem[Float].sqrtT`), `abs` under IEEE `<` (so
`-0.0` and NaN survive — `Numeric[Float].lt` is IEEE while `Ordering[Float]` is total),
`matmul` as the sequential k-sum per cell that `multiplyFloat`'s tiling produces, LU with
the pivot by `|toDouble|`. Deterministic single precision, so 286 `mf.*` rows pin raw
`floatToIntBits`. Not pinned: `exp`/`log` (through `Math.exp`); not present: the
Double-only decompositions, and Scala's `Mat[Float]` BLAS route (`matmul` is always the
pinned loop here).

**Mat[Big]: the exact-decimal matrix, digit-for-digit.** On the Scala side `Mat[Big]` is
the generic `Mat[T]` with `Fractional[Big]`: no `fastD` branch, so every reduction is the
plain sequential row-major fold and every elementwise op the guarded `Big` operator. The
Rust `MatB` reproduces those folds — `sum mean std variance norm cumsum` and the axis
family from `Big(0)`, `power` as repeated multiplication from `Big(1)`, `matmul` as a
sequential k-sum, LU `inverse`/`determinant`/`solve` with the pivot chosen by `|toDouble|`
— and the fixture pins results as `BigDecimal.toString` text, scale included (293 `bm.*`
rows on shapes up to 64×64, a NaN-injected variant alongside). Rules pinned there:
`BigNaN` orders highest (`compare`), so `sort` puts it last, `min` skips it, `max` returns
it; the ordering masks `gt lt gte lte` are false against it (guarded operators) while
`:==`/`hasNaN` recognise it; arithmetic and every fold propagate it. `exp`/`log` go
through `Math.exp` on both sides and are not fixture-pinned (libm). `svd eig cholesky
lstsq pinv matrixRank` are Double-only in Scala and absent from `MatB`. Two `Big::sqrt`
defects surfaced only here — the scalar fixture never hit them — and are fixed: the result
now strips trailing zeros down to the preferred scale as `BigDecimal.sqrt(mc)` does, and
an inexact root rounds ONCE on the most significant dropped digit rather than digit by
digit (which double-rounded a `…49` tail up).
