...
| `uproc` | `uni.Proc` — `run`, `proc`, `ProcResult`, `ProcBuilder`, `bashExe`/`pythonExe`/`whereInPath`/`uname` | the same routing table by file extension; `where` is `whereExe`, `failFast` is `?` |
...
| `market_sim` — `src/market_sim.rs`, library code (the `market-sim` feature) with its binary in `src/bin/`, not an example; the twin is `../jsrc/marketSim.sc` | not a tour of an API but a 1,300-line workload: seeded price formation over 200 paths × 100 years, `MatD` reductions, drawdown episodes, exposure rules, and five report modes — the consumer that drove the `MatD` port |
