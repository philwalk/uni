...
class MatTest extends munit.FunSuite {
  ...
  test("rand Mat RNG matches NumPy seed=42 integer sequence") {
    ...
    // Assertion: compare the sequences
    assertEquals(actual.toList, expected.toList, "The random long sequence should exactly match the NumPy seed=42 reference")
  }
  ...
  */
  ...
  test("Boolean Indexing (Filtering)") {
    val m = MatD.row(1.0, 2.0, 3.0, 4.0)
    val mask = m.gt(2.0) // Expected: [false, false, true, true]
    val filtered = m(mask)
    
    assertEquals(filtered.rows, 1)
    assertEquals(filtered.cols, 2)
    assertEquals(filtered(0, 0), 3.0)
    assertEquals(filtered(0, 1), 4.0)
  }

  test("Logical Operations (AND, OR, NOT)") {
    val v = MatD.row(1.0, 2.0, 3.0, 4.0)
    val mask1 = v.gt(1.0) // [F, T, T, T]
    val mask2 = v.lt(4.0) // [T, T, T, F]
    
    val combinedAnd = mask1 && mask2 // Expected: [F, T, T, F]
    val combinedOr  = mask1 || mask2 // Expected: [T, T, T, T]
    val negated     = !mask1         // Expected: [T, F, F, F]
    
    assertEquals(combinedAnd.sum.toInt, 2, "AND logic should find 2 elements")
    assert(combinedOr.all, "OR logic should be all true")
    assert(negated.at(0, 0), "NOT logic failed at index 0")
  }

  test("Conditional Selection (where)") {
    val v = MatD.row(-1.0, 2.0, -3.0, 4.0)
    val mask = v.lt(0.0)
    // Replace negatives with absolute value
    val result = MatD.where(mask, v.abs, v) 
    
    val allPositive = result.all(x => x > 0)
    assert(allPositive, "Where condition should have resulted in all positive values")
    assertEquals(result(0, 0), 1.0)
    assertEquals(result(0, 2), 3.0)
  }

  test("Reductions (all, any)") {
    val m = MatD.ones(2, 2)
    val allTrue = m.gt(0.0)
    
    // Using explicit Mat[Boolean] construction if your factory supports it
    // otherwise creating from a comparison as above
    assert(allTrue.all, "all() should be true for ones.gt(0)")
    
    val someTrue = MatD.row(1.0, 0.0, 0.0).gt(0.5)
    assert(someTrue.any, "any() should be true if one element is true")
    assert(!someTrue.all, "all() should be false if some elements are false")
  }
}
