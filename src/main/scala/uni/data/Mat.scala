...
object Mat {
  ...
  private inline def fastD[T](m: Mat[T])(using ct: ClassTag[T]): Boolean =
...
//      m.tdata(r * m.cols + c) = value
  ...
  private def nanFill[T](using elem: MatElem[T]): T = elem.nan
...
// Then use getA(i, k) inside the loops
  ...
  // widening without any explicit Conversion. No given cvecToMat needed.
  object CVec:
    ...
    def fromMat[T](m: Mat[T]): CVec[T] =
      ...
      if m.cols == 1 then m else Internal.transposeView(m.asInstanceOf[Internal.MatData[T]])
    // ClassTag-free widening.  Used from VecExts.scala (outside object Mat, where
    // CVec[T] is opaque and `(cv: Mat[T])` upcast is not available without <: bound).
    extension [T](v: CVec[T])
    ...
    extension [T](v: CVec[T])(using @annotation.unused ct: ClassTag[T])
      ...
      def norm(using frac: Fractional[T], elem: MatElem[T]): T = {
        ...
        if fastD(mat)
        then
          val a = mat.tdata.asInstanceOf[Array[Double]]
          var sumSq = 0.0; var i = 0
          while i < a.length do { val x = a(i); sumSq += x * x; i += 1 }
          math.sqrt(sumSq).asInstanceOf[T]
        else
  ...
  // ── RVec companion ──────────────────────────────────────────────────────────
  // RVec[T] <: Mat[T] (opaque subtype), so widening is automatic. No given needed.
  object RVec:
    ...
    def fromMat[T](m: Mat[T]): RVec[T] =
      require(m.rows == 1 || m.cols == 1,
        s"fromMat requires a vector-shaped matrix, got ${m.rows}×${m.cols}")
      if m.rows == 1 then m else Internal.transposeView(m.asInstanceOf[Internal.MatData[T]])
    // ClassTag-free widening.  Used from VecExts.scala (outside object Mat).
    extension [T](@annotation.unused rv: RVec[T])
...
object MatD {
  ...
  def col(values: Double*): MatD = Mat.create(values.toArray, values.length, 1)
...
object MatB {
  ...
  def col(values: Double*): MatB = Mat.create(values.map(Big(_)).toArray, values.length, 1)
  ...
  def rand(rows: Int, cols: Int): Mat[Big] = {
    val rng  = Mat.globalRNG
    val data = Array.fill(rows * cols)(Big(rng.nextDouble()))
    Mat.create(data, rows, cols)
  }
  def randn(rows: Int, cols: Int): Mat[Big] = {
    val rng  = Mat.globalRNG
    val data = Array.fill(rows * cols)(Big(rng.randn()))
    Mat.create(data, rows, cols)
  }
  def uniform(low: Double, high: Double, rows: Int, cols: Int): Mat[Big] = {
    val rng  = Mat.globalRNG
    val data = Array.fill(rows * cols)(Big(rng.uniform(low, high)))
    Mat.create(data, rows, cols)
  }
  def normal(mean: Double, std: Double, rows: Int, cols: Int): Mat[Big] = {
    val rng  = Mat.globalRNG
    val data = Array.fill(rows * cols)(Big(mean + std * rng.randn()))
    Mat.create(data, rows, cols)
  }
  def randint(low: Int, high: Int): Int = Mat.randint(low, high)
  def randint(low: Int, high: Int, rows: Int, cols: Int): Mat[Int] = Mat.randint(low, high, rows, cols)
...
object MatF {
  ...
  def col(values: Double*): MatF = Mat.create(values.map(_.toFloat).toArray, values.length, 1)
  ...
  def rand(rows: Int, cols: Int): Mat[Float] = {
    val rng  = Mat.globalRNG
    val data = Array.fill(rows * cols)(rng.nextDouble().toFloat)
    Mat.create(data, rows, cols)
  }
  def randn(rows: Int, cols: Int): Mat[Float] = {
    val rng  = Mat.globalRNG
    val data = Array.fill(rows * cols)(rng.randn().toFloat)
    Mat.create(data, rows, cols)
  }
  def uniform(low: Double, high: Double, rows: Int, cols: Int): Mat[Float] = {
    val rng  = Mat.globalRNG
    val data = Array.fill(rows * cols)(rng.uniform(low, high).toFloat)
    Mat.create(data, rows, cols)
  }
  def normal(mean: Double, std: Double, rows: Int, cols: Int): Mat[Float] = {
    val rng  = Mat.globalRNG
    val data = Array.fill(rows * cols)((mean + std * rng.randn()).toFloat)
    Mat.create(data, rows, cols)
  }
  def randint(low: Int, high: Int): Int = Mat.randint(low, high)
  def randint(low: Int, high: Int, rows: Int, cols: Int): Mat[Int] = Mat.randint(low, high, rows, cols)
