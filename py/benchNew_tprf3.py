...
def run(label: str, T: int, N: int, L: int, warmup: int, loops: int):
    ...
    ms_rec_orig = bench(oos_loops,
                        lambda: estimate_orig(y, X, Z, procedure="OOS Recursive", mintrain=T // 2))
    ms_rec_fast = bench(oos_loops,
                        lambda: estimate_fast(y, X, Z, procedure="OOS Recursive", mintrain=T // 2))
    ...
    # ── OOS Cross Val ────────────────────────────────────────────────────────
    ms_cv_orig = bench(oos_loops,
                       lambda: estimate_orig(y, X, Z, procedure="OOS Cross Val"))
    ms_cv_fast = bench(oos_loops,
                       lambda: estimate_fast(y, X, Z, procedure="OOS Cross Val"))
