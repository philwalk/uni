...
`rust/examples/<name>.rs` (`marketSim`'s twin is library code, `rust/src/market_sim.rs`, with
its binary in `src/bin/`), written to read line-for-line alike — and the two print
...
| `marketSim` | the odd one out: a full simulation workload rather than an API tour. Seeded price formation over 200 paths × 100 years, `MatD` reductions, drawdown episodes, exposure rules with a two-heap running median, and the report modes — `-emit`/`-validate`/`-fitness`/`-strategies`/`-power`/`-buffer`/`-crossasset`/`-releases` — plus `-calibrate` | identical **everywhere** |
...
```scala
libraryDependencies += "org.vastblue" %% "uni" % "0.24.1"
```
