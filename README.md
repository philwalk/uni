...
Note on `matmul`: since 0.16.1 the **default** `*@` is the pure tiled loop — bit-identical
across the Scala and Rust ports and across machines — and BLAS is an opt-in
(`-Duni.mat.blas=true`). NumPy's `@` is always OpenBLAS, so this row compares a
reproducible loop against a native kernel; with the opt-in, MatD is level with NumPy here.
The Linux and macOS tables below were measured under the former BLAS-by-default
behaviour, which is why their matmul rows are level without a flag. The current
