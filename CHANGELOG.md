...
- The swing never carries the price nearer than 0.10 log to the running peak, measured from the
  ...
  macro build-up band from 0.10. Once the price regains the running peak while the state is armed
  the unwind is over and the state fades with a one-month half-life: without that the swing ran
  on for up to a year after a full recovery and its downward moves minted 20% peaks under a
  still-depressed conditions index (two a century at 0.20 against 0.9 with the swing off; a
  relief fading near the peak and beliefs blind to the swing left them untouched), which is what
...
- A verdict row's `real` is now the record, read the way the model reads a path, on every row one
  daily record can be read that way (twelve per set), and a new `target` carries what the loss
  grades against. They differ where the target is a theory value — the 60-day variance ratio's
  1.00, where QQQ reads 0.83 and CRSP 1.01 — or older than its record: S&P vol 16.0 against 15.68,
  kurtosis 28 (the century's) against 1954-2026's 21.8, crashes 20.7 against 24.9, median depth
  −21.4 against −20.8, and the Nasdaq downside excess 1.13 against 1.07. The targets are
  unchanged. A consumer that divided `model` by `real` on the variance ratio read a theory value
  as a bias.
...
- `typical-year vol %` is anchored on the median-year vol averaged over all 252 block phases: QQQ
  ...
  keep their widths around the new anchors (16.4-23.6, 10.9-14.1) and both shipped worlds pass
